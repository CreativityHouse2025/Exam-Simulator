    ...
    <Card id={`question-${number}`} className="scroll-mt-28 md:scroll-mt-4">
      <CardContent className="flex flex-col gap-4">
        <div className="flex items-start gap-3">
          <span className="shrink-0 rounded-md bg-primary/10 px-2 text-sm font-bold text-primary">
            {number}
          </span>
          <p className="mt-0 text-sm font-semibold text-tertiary">
            {question.text}
          </p>
        </div>
          ...
          <CollapsibleContent className="mt-3 flex flex-col gap-2">
            {question.choices.map((choice, i) => (
              <ChoiceRow
                key={i}
                choice={choice}
                letter={formatChoiceLabel(i, settings.language)}
              />
            ))}
          </CollapsibleContent>
        </Collapsible>
        ...
        {question.explanation && (
          <Collapsible
            ...
            onOpenChange={() => onToggle("explanation")}
          >
