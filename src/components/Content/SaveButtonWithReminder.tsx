...
import { useSessionData, useSessionControl } from '../../contexts'
...
`
const SaveButtonWithReminder: React.FC = () => {
  const { isSyncing, dirtyQuestions } = useSessionData()
  const { syncProgress } = useSessionControl()

  const dirtyCount = Object.keys(dirtyQuestions).length
