...
const ModalComponent: React.FC<ModalProps> = ({ title, message, buttons, onConfirm, onClose, variant }) => {
  ...
  return (
    <div id="modal-cover" className="fixed top-0 left-0 z-5 w-full h-full bg-black/50" onClick={handleBackdropClick}>
      <div
        id="modal-window"
          ...
          <div id="buttons" className="h-12.5 flex items-center justify-center border-t border-grey-200 bg-grey-50">
            <button
              ...
              className={cn(
                ...
                variant === 'danger' ? "bg-danger hover:bg-danger-hover" : "bg-secondary hover:bg-secondary-hover",
              )}
              onClick={onConfirm}
            >
              {buttons[0]}
            ...
            {onClose && (
              <button
                id="button-cancel"
                className={cn(BUTTON_BASE, "no-select text-grey-950 bg-grey-200 hover:bg-grey-300")}
                onClick={onClose}
              >
                {buttons[1]}
              </button>
