...
const BTN_BASE =
  "flex items-center justify-center font-bold text-base uppercase py-2 px-2.5 rounded-xs transition-colors duration-300 cursor-pointer";
...
export default function BreakOfferModal({
  title,
  message,
  primaryLabel,
  secondaryLabel,
  ...
  return (
    <ModalOverlay onClick={handleBackdropClick}>
      <div className={BREAK_CARD_CLASSES}>
        ...
        <div className="flex items-center justify-center text-2xl font-semibold py-6 px-5 text-center font-sans">
          {message}
        </div>
        <div className="flex items-center justify-center gap-2.5 pt-0 px-3 pb-3 border-t border-grey-200 bg-grey-50 min-h-12.5">
          ...
          <button
            className={cn(
              BTN_BASE,
              "text-grey-950 bg-grey-200 hover:bg-grey-300",
            )}
            onClick={onSkip}
          >
            {secondaryLabel}
