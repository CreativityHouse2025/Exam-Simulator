...
const RevisionTopDisplay: React.FC<TopDisplayProps> = ({ questionCount, isReview = false }) => {
  const { index } = useExamSessionCore()
  const { examId } = useRevisionExamSession()
  const examLabel = useFullExamLabel(examId)
...
export default RevisionTopDisplay
