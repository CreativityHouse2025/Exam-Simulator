...
import DomainExamConfirms from './Confirms'
import DomainExamSession from './DomainExamSession'
...
/**
 * Loads the domain exam JSON, applies question choice orders, and provides exam data to
 * the domain exam session tree via ExamContext. Renders timer/pause confirms.
 *
 * Re-loads on language change.
 */
export default function DomainExamProvider() {
  ...
  React.useEffect(() => {
    ...
    async function loadExamData() {
      const examDetails = await loadDomainExam(session!.categoryId!, langCode)
      if (cancelled) return
  ...
  }, [session?.categoryId, session?.questionIds, langCode])
