...
const ChoiceComponent: React.FC<ChoiceProps> = ({
  ...
  const IconComponent = selected
    ? iconComponents.selected
    : iconComponents.unselected;
  ...
  return (
    <div
      className={cn(
        "choice-grid grid items-center mb-1.25 text-sm md:text-base",
    ...
    >
      <IconComponent
        className={cn("no-select mr-1.25", colorClass)}
        size={selected ? 22 : 20}
      />

      {/* margin to align with radio buttons (Open Sans issue) */}
      <div
        className={cn("-mt-0.5 justify-self-center font-semibold", colorClass)}
      >
        {label}
      </div>
