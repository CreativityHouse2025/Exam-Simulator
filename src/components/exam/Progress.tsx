...
const ProgressComponent: React.FC<ProgressProps> = ({ questionCount }) => {
  const { selectedChoices } = useExamSession();

  const { answeredCount, percentage } = React.useMemo(() => {
    return calculateProgressStats(questionCount, selectedChoices);
  }, [questionCount, selectedChoices]);

  return (
    <div
      id="progress"
      className="no-select flex flex-col gap-1.25 p-2.5 rounded-sm mb-2.5"
    >
      <span className="text-xl font-bold text-primary">{`✍️ ${answeredCount} (${percentage}%)`}</span>

      <div
        id="progress-bar"
        className="w-full h-1.25 bg-secondary rounded-sm overflow-hidden"
      >
        <div
          id="progress-fill"
