...
const MultipleChoiceComponent: React.FC<MultipleChoiceProps> = ({ isReview, isAnswerRevealed = false }) => {
  const { question, index: questionIndex, selectedChoices, setAnswer } = useExamSession()
  ...
  const onChoose = React.useCallback(
    (position: number) => {
      ...
      if (isAlreadySelected) {
        setAnswer(questionIndex, selectedPositions.filter((selected) => selected !== position))
      } else if (!maxAnswersReached) {
        setAnswer(questionIndex, [...selectedPositions, position])
      }
    },
    [questionIndex, selectedPositions, isChoiceInteractionLocked, isSingleAnswer, maxAnswersReached, setAnswer]
  )
