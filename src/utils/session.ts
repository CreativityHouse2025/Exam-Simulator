...
function applyOffered(
  state: Session,
  action: SessionAction<"SET_OFFERED_BREAK">,
): Session {
...
export const SessionReducer: SessionReducerFunc = (
  state: Session | null,
  ...
  if (!Array.isArray(actions)) {
    ...
    if (type === "MARK_DIRTY") {
      ...
      return {
        ...state,
        dirtyQuestions: { ...state.dirtyQuestions, [questionIndex]: true },
      };
    }
  ...
  const reset = actions.find(
    (action): action is SessionAction<"RESET_SESSION"> =>
  ...
  let newState: Session = reset
    ? (reset.payload as Session)
    : (state as Session);
  ...
  for (const action of actions) {
    ...
    if (type === "MARK_DIRTY") {
      ...
      if (!newState.dirtyQuestions[questionIndex]) {
        newState = {
          ...newState,
          dirtyQuestions: { ...newState.dirtyQuestions, [questionIndex]: true },
        };
        hasChanges = true;
      }
      continue;
    ...
    if (type === "SET_OFFERED_BREAK") {
      const updated = applyOffered(
        newState,
        action as SessionAction<"SET_OFFERED_BREAK">,
      );
      if (updated !== newState) {
        newState = updated;
        hasChanges = true;
