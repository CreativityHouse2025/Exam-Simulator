...
export function validateSaveAttempt(body: unknown): SaveAttemptRequestBody {
  ...
  const parsedAnswers: SaveAttemptAnswer[] = answers.map((entry, i) => {
    ...
    if (!isNonNegativeInteger(question_index)) {
      throw new AppError({ statusCode: 400, code: "VALIDATION_ERROR", message: `answers[${i}].question_index must be a non-negative integer, got ${question_index}` })
    }
    if (!Array.isArray(selected_choices) || !selected_choices.every(isNonNegativeInteger)) {
      throw new AppError({ statusCode: 400, code: "VALIDATION_ERROR", message: `answers[${i}].selected_choices must be an array of non-negative integers, got ${JSON.stringify(selected_choices)}` })
    }
    if (typeof is_bookmarked !== "boolean") {
      throw new AppError({ statusCode: 400, code: "VALIDATION_ERROR", message: `answers[${i}].is_bookmarked must be a boolean, got ${JSON.stringify(is_bookmarked)}` })
    }
