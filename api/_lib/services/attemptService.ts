...
export async function insertAttempt(
  userId: string,
  input: InsertAttemptRequestBody,
  ...
  const questions = question_ids.map((question_id, i) => ({
    question_id,
...
export async function saveAttempt(
  ...
  const { data: result, error } = await supabaseAdmin.rpc("save_attempt", {
    ...
    p_break_1_offered_at:
      input.exam_state === "in-progress" ? input.break_1_offered_at : null,
    p_break_2_offered_at:
      input.exam_state === "in-progress" ? input.break_2_offered_at : null,
  });
...
export async function listAttempts(
  ...
  const { data, error } = await supabaseAdmin
    .from("exam_attempts")
    .select(
      "id, exam_type, exam_id, category_id, exam_state, score, status, created_at, time_remaining, exam_attempt_questions(count)",
    )
    .eq("user_id", userId)
  ...
  const attempts = (data ?? []).map(
    ({ exam_attempt_questions, ...attempt }) => ({
      ...attempt,
      total_questions: exam_attempt_questions[0]?.count ?? 0,
    }),
  );

  return { attempts: attempts as ListAttemptsResult["attempts"] };
}
...
export async function getAttempt(
  ...
  const { data, error } = await supabaseAdmin
    ...
    .order("question_index", {
      referencedTable: "exam_attempt_questions",
      ascending: true,
    })
    .single();
  ...
  const attempt = { ...attemptRow, total_questions: questions?.length ?? 0 };

  return {
    attempt: attempt as GetAttemptResult["attempt"],
    questions: questions ?? [],
  };
}
